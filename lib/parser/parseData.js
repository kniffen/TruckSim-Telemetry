function parseData(data) {
  ...
  if ( output.game.pluginVersion >= 12 )
    output.game.multiplayerTimeOffset.value = parseInt( output.game.multiplayerTimeOffset.value.reduce(reducer) / 1000 )
  ...
  if ( output.game.pluginVersion >= 11 ) {
    output.truck.lights.hazard.enabled = convertIntToBool( output.truck.lights.hazard.enabled )
  }

  // Truck: differential lock
  if ( output.game.pluginVersion >= 11 ) {
    output.truck.differential.lock.enabled = convertIntToBool( output.truck.differential.lock.enabled )
  }

  // Truck: liftable axle
  if ( output.game.pluginVersion >= 11 ) {
    output.truck.liftAxle.enabled           = convertIntToBool( output.truck.liftAxle.enabled )
    output.truck.liftAxle.indicator.enabled = convertIntToBool( output.truck.liftAxle.indicator.enabled )
  }
  ...
  for ( let i = 0; i < tmpTrailers.length; i++ ) {
    ...
    if ( output.game.pluginVersion >= 11 ) {
      ...
      tmpTrailers[i].liftAxle.enabled           = convertIntToBool( tmpTrailers[i].liftAxle.enabled)
      tmpTrailers[i].liftAxle.indicator.enabled = convertIntToBool( tmpTrailers[i].liftAxle.indicator.enabled)
    }
