...
function getDataFromPluginVersion10(buffer) {
  ...
  const entries = _.struct([
    ...
    // 11th zone
    _.uint32le("events.job.penalty"),
    _.uint32le("events.job.revenue"),
    _.uint32le("events.fine.amount"),
    _.uint32le("events.tollgate.amount"),
    _.uint32le("events.ferry.amount"),
    _.uint32le("events.train.amount"),
