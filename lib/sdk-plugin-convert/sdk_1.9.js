...
function getDataFromPluginVersion9(buffer) {
  ...
  const entries = _.struct([
    ...
    _.struct("trailer.hook.position", [
       _.float32le("X"),
       _.float32le("Y"),
       _.float32le("Z"),
    ]),

    _.float32le("trailer.wheels.position.X", 16),
    _.float32le("trailer.wheels.position.Y", 16),
    _.float32le("trailer.wheels.position.Z", 16),
    ...
    // 11th zone
    _.uint8("job.active"),
    _.uint8("job.finished"),
    _.uint8("trailer.hasCargo"),
